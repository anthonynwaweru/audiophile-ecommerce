...
const Inputs = () => {
  ...
  const schema = yup.object().shape({
    ...
    eMoneyPin: yup.string().when("requirePIN", () => {
      if (isEMoney) {
        return yup
          .number()
          .typeError("Must be a number")
          .test("len", "Must be 4 characters", (val) =>
            val ? val.toString().length === 4 : null
          );
      }
    }),
  });
